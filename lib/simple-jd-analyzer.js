class JobDescriptionAnalyzer {
  ...
  extractJobTitle(text) {
    ...
    const titlePatterns = [
      ...
      /position[:\s]+(.*)/i,
      /role[:\s]+(.*)/i,
      /looking for.*?([A-Z][a-z-\s]+Engineer[^.]*)/i,
      /seeking.*?([A-Z][a-z-\s]+Engineer[^.]*)/i,
      /hiring.*?([A-Z][a-z-\s]+Engineer[^.]*)/i
    ];
    
    // Look for specific job title patterns in the text
    const jobTitleMatch = text.match(/looking for.*?(Back-End Engineer[^.]*?\([^)]*\))/i);
    if (jobTitleMatch) {
      return jobTitleMatch[1].trim();
    }
    
    // More general pattern for job titles with parentheses
    const generalTitleMatch = text.match(/looking for.*?([A-Z][a-z-\s]+Engineer[^.]*?\([^)]*\))/i);
    ...
    for (let i = 0; i < Math.min(10, lines.length); i++) {
      ...
      if (line.length > 5 && line.length < 100) {
        const commonTitles = [
          ...
          'junior', 'principal', 'staff', 'back-end', 'front-end'
        ];
  ...
  extractCompany(text) {
    // Look for "About [CompanyName]" pattern first
    const aboutMatch = text.match(/About\s+([A-Za-z0-9\s]+)/i);
    if (aboutMatch) {
      const companyName = aboutMatch[1].trim().split('\n')[0];
